...
if list_requests:
    for torrent in list_requests:
        cursor.execute(
            f"SELECT url, title, release_date FROM torrent "
            f"WHERE id = '{torrent}'"
        )
        ...
        if data:
            ...
            driver.get(f'https://rarbgproxied.org{data[0]}')
            ...
            extension = biggest.path.split('.')[-1]
            file_name = f'{str(Path.home() / "Videos")}/{data[1]}.{extension}'
            handle.rename_file(biggest_index, file_name)
